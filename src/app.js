...
app.get("/", (req, res) => res.send("____Laboratoireservice API is running 🚀"));
...
connectDB()
  .then(async () => {
    ...
    app.listen(PORT, () =>
      console.log(`✅ Laboratoireservice Server running on port ${PORT}`)
    );
    
  })
  .catch((error) => {
    console.error("❌ Laboratoireservice Database connection failed:", error.message);
    process.exit(1);
  });
